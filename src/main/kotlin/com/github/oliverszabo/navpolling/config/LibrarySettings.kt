...
class LibrarySettings(
    ...
    private fun initializePollingFrequency(): Trigger {
        ...
        val specifiedPollingFrequencyParts = specifiedPollingFrequency.trim().replace("\\s+".toRegex(), " ").split(" ")
        if(specifiedPollingFrequencyParts.size == 2
            ...
            return PeriodicTrigger(specifiedPollingFrequencyParts[0].toLong(), TimeUnit.valueOf(specifiedPollingFrequencyParts[1]))
        }

        try {
            return CronTrigger(specifiedPollingFrequency)
